...
from src.eval.langsmith_sync import log_eval_summary, upload_dataset
...
@click.option(
    "--upload",
    is_flag=True,
    help="Upload gold set and summary metrics to LangSmith.",
)
@click.option(
    "--report",
...
def eval_run_command(
    ...
    if upload:
        dataset_id = upload_dataset(cases)
        log_eval_summary(eval_report)
        console.print(
            f"Uploaded dataset to LangSmith (id={dataset_id}).",
            style="dim",
        )
